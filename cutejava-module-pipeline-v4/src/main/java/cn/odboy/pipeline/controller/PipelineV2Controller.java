...
import cn.odboy.pipeline.core.PipelineOrchestrator;
...
public class PipelineV2Controller {
  ...
  @Autowired
  private cn.odboy.pipeline.core.TaskScheduler taskScheduler;
  ...
  public ResponseEntity<?> stopPipeline(@RequestBody PipelineInstanceTb args) {
    try {
      // 从数据库加载实例信息
      PipelineInstanceService instanceService = KitSpringBeanHolder.getBean(cn.odboy.pipeline.service.PipelineInstanceService.class);
