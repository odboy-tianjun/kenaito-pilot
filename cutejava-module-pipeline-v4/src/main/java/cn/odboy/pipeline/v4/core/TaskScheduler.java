...
import cn.odboy.pipeline.v4.util.PipelineNameUtil;
...
public class TaskScheduler {
  ...
  /**
   * 停止 Job
   * 注意：在集群模式下，scheduler.interrupt() 会自动将中断信号发送到运行该 Job 的节点
   */
  public void stopJob(TaskContext context) {
    try {
      ...
      // 先尝试在本机设置中断标志（如果任务恰好在本机运行）
      TaskJob.setInterruptFlag(context.getTaskId());

      // Quartz 集群会自动将中断信号发送到运行该 Job 的节点
      // 被中断的节点的 TaskJob.interrupt() 会被调用
