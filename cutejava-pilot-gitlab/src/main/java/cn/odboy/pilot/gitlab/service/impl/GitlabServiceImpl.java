...
public class GitlabServiceImpl implements InitializingBean, GitlabService {
  ...
  public void startPipelineWithListener(@NonNull String projectEnglishName, @NonNull String branchName, Map<String, String> variables, GitlabPipelineListener listener) {
    Pipeline pipeline = this.createPipelineByProjectName(projectEnglishName, branchName, variables);
    taskExecutor.execute(() -> GitlabServiceImpl.this.waitForPipelineCompletion(pipeline, listener));
  }
   ...
   * 同步等待流水线执行完成
   * <p>
   * 该方法会阻塞当前线程，直到流水线执行完成（成功或失败）或超时。
   * 执行过程分为两个阶段：
   * 1. 等待阶段：检测流水线是否从 PENDING 状态变为 RUNNING 状态，若超过最大重试次数则抛出异常
   * 2. 监控阶段：持续监控流水线执行状态，直到成功或失败，若超过最大检查次数则抛出超时异常
   *
  ...
  private Namespace getRootNamespace() {
    return this.getNamespaceById(properties.getRootId());
  }
